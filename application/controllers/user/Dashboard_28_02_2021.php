...
public function benefitpay_request()
  {
    ...
    $Pipe->setresponseURL(base_url().'user/Dashboard/benefitpay_response');
    $Pipe->seterrorURL(base_url()."user/Dashboard/error");
    ...
    if ($trandata != "")
    {
      ...
      if ($returnValue == 1)
      {
        ...
        else if ($Pipe->getresult() == "NOT CAPTURED" || $Pipe->getresult() == "CANCELED" || $Pipe->getresult() == "DENIED BY RISK" || $Pipe->getresult() == "HOST TIMEOUT")
        {
          ...
          echo "REDIRECT=".base_url()."declined.php";
        }
        else
        {
          //Unable to process transaction temporarily. Try again later or try using another card.
          echo "REDIRECT=".base_url()."user/Dashboard/err_response";
        }
      }
      else
      {
